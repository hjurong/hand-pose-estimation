...
def pso(func, x0, std, lb, ub, ieqcons=[], f_ieqcons=None, args=(), kwargs={}, 
    ...
    x0: array
        initial guess
    std: array
        distribution of particle initialisation
    lb : array
    ...
    x = np.random.randn(S, D)  # particle positions
...
#        v[i, :] = vlow + np.random.rand(D)*(vhigh - vlow)
    ...
    while it<=maxiter:
        ...
        for i in xrange(S):
            ...
            # gradient descent
#            x[i, :] = optimize.fmin_cg(func, x[i, :], args=args, maxiter=7)

            # Update the particle's velocity
            v[i, :] = omega*v[i, :] + phip*rp[i, :]*(p[i, :] - x[i, :]) + \
                      phig*rg[i, :]*(g - x[i, :]) # original
