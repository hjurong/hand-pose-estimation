...
def thumb_model(theta_ij, g_pos, tb_geometry, 
                spacing = 0, gb_trans = np.deg2rad([30, 30])):
...
def hand_model(hand_params, hand_geometry, ref_pos):
    ...
    thumb = thumb_model(thumb_the, ref_pos, thumb_geo, spacing=1)
    index = finger_model(index_the, ref_pos, index_geo, spacing=0.5)
    middle = finger_model(middle_the, ref_pos, middle_geo, spacing=0.)
    ring = finger_model(ring_the, ref_pos, ring_geo, spacing=-0.5)
    little = finger_model(little_the, ref_pos, little_geo, spacing=-1.)
...
def build_spheres(hand_joints, sphereNum=[4,2,2,2], drw_spheres=False):
    """
...
if __name__ == '__main__':
	...
	# joints = hand_model(the_params, geo_params, gpos)
	# cen, rdi = build_spheres(joints)

	#visualise_hand_model(the_params, geo_params, gpos)
	#plt.show()
