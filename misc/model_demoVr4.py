...
from read_binary import get_depthmap, project_spheres, load_sphere_txt
from read_binary import load_joints_txt
...
def finger_model(thetas, g_pos, f_geometry, 
                 spacing = 0.5, gb_trans = np.deg2rad([30, 30, 30])):
...
#    Tgb = np.array([c(TWS), -s(TWS)*c(ROT), s(TWS)*s(ROT), 0,
#                    s(TWS),  c(TWS)*c(ROT),-c(TWS)*s(ROT), 0,
#                          0,        s(ROT),        c(ROT), 0,
#                          0,             0,             0, 1]).reshape(4,4)
...
def hand_model(hand_params, hand_geometry, ref_pos, global_T):
    ...
    thumb = thumb_model(thumb_the, ref_pos, 
                        thumb_geo, spacing=1.1, gb_trans=global_T)
...
def build_spheres2(hand_joints, fg_num=[4,2,2,2], tb_num=[2,2,2,2], 
                   draw = False):
    ...
    Sradius = load_sphere_txt() # unit == mm
    Sradius /= 10.
...
#        z = np.outer(np.cos(u), np.ones_like(v))
    ...
    def update(val):
        ...
        chand_params = chand_params.flatten()

        nhand_joints = hand_model(chand_params, hand_geo, ref, gtrans)
...
def match_models(spheres_center, radii, plot=False):
    ## spheres_center has unit cm; shape = (nspheres, 3)
    depth, ptncloud, scale = get_depthmap()
    ## ptncloud.shape = (3, nptn) --> need to take transpose
...
#    print ptncloud.shape, spheres_center.shape
    ...
    const = float(spheres_center.shape[0]) / ptncloud.shape[0]
    total = sum(dist) * const # apply the scaling
...
#    gpos = [5,0,5]
    gpos = np.array([0, 4, 32])
    global_trans = np.deg2rad([90,0,-40])
    ...
    #    a = a + " << " + str(elem)
    # print a
    gbpos, hdgeo, cmcth = load_joints_txt() # unit = mm
    gbpos /= -10.
    hdgeo = hdgeo.flatten() / 10.
    

#    joints = hand_model(the_params, hdgeo, gpos, global_trans) #geo_params, gpos,
#    cen, rdi = build_spheres2(joints)
#    # print joints
...
#    print tdist
#    
#    depthmp, pcloud, pscale = get_depthmap()
#    BcD = project_spheres(cen.T, depthmp, pscale)

    
    visualise_hand_model(the_params, hdgeo, gpos, global_trans) # geo_params, gpos,
    plt.show()
